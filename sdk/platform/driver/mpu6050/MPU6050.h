...
enum {
	...
	MPU6050_REG_CHIP_ID = 0x75,
};

bool mpu6050_set_enable(bool enable);
bool mpu6050_read_sensor_values(uint8_t values[3]);

static inline int mpu6050_read_data(uint8_t addr, uint8_t *data, int size)
{
	return i2c_read_data(MPU6050_I2C_ADDRESS, addr, data, size);
}

static inline int mpu6050_write_data(uint8_t addr, const uint8_t *data, int size)
{
	return i2c_write_data(MPU6050_I2C_ADDRESS, addr, data, size);
}

static inline int mpu6050_read_register(uint8_t addr, uint8_t *value)
{
